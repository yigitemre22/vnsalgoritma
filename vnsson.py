...
# ---------------------------------------------------------
# 3. ANA PROGRAM (IndexError Düzeltildi ve Çıktı Sadeleştirildi)
# ---------------------------------------------------------
...
if __name__ == "__main__":
    ...
    print("="*100)
    print("Her talep 5 farklı rastgele amaç ağırlığı ile test ediliyor. Yalnızca en iyi sonuç listelenmiştir.")
    ...
    # Başlık Çizgisi Uzunluğu Ayarlandı
    print("\n" + "="*120)
    print("| GENEL BAŞARI ÖZETİ (Tüm Taleplerin 5 Deneme Arasındaki En İyi Çözümü) |")
    print("="*120)
    
    # 🔥 Düzeltilmiş Header Dizesi: Sadece 7 Yer Tutucu.
    # Talep(1) | Src(2)->Dst(3) | Maliyet(4) | Gecikme(5) | Güvenilirlik(6) | Kullanım(7)
    header = "{:<5} | {:<5}->{:<5} | {:<7} | {:<10} | {:<13} | {:<10}"
    # Argümanlar: Talep, Src, Dst, Maliyet, Gecikme, Güvenilirlik, Kullanım (7 argüman)
    print(header.format("Talep", "Src", "Dst", "Maliyet", "Gecikme(ms)", "Güvenilirlik", "Kullanım"))
    print("-" * 120)
    ...
    for result in all_results:
        ...
        if best['path'] and best['cost'] != float('inf'):
            ...
            # 1. SATIR: Temel Metrikler (7 argüman ile 7 yer tutucuyu doldurur)
            print(header.format(
                result['demand_id'],
                result['source'],
                result['target'],
                ...
                f"{best['delay']:.2f}",
                f"%{rel_percent:.2f}",
                f"{best['resource_usage']:.2f}" # Son argüman
            ))
            
            # 2. SATIR: Ağırlıklar ve Yol Özeti
            ...
            # Ağırlıklar ve Yol bilgisi kısa ve girintili bir satırda
            print(f"{' '*20} [Wgt: {best['weights']}] | Yol: {path_str}")
        else:
            # Başarısız sonuç
